...
def generate_local_ollama_report(prompt: str, model: str = 'llama3') -> str:
    ...
    command = ['ollama', 'run', '--nowordwrap', model]
    process = subprocess.Popen(command, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    try:
        stdout, stderr = process.communicate(input=prompt, timeout=120)
        if process.returncode != 0:
            raise RuntimeError(f'Ollama generation failed: {stderr.strip()}')
        return stdout.strip()
    except subprocess.TimeoutExpired:
        process.kill()
        raise RuntimeError('Ollama generation timed out after 120 seconds')
